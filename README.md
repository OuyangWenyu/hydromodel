...
- 《水文预报》书上有个公式——$FR_{\frac{\Delta t}{N}}=1-(1-FR)^{\frac{1}{N}}$：
   ...
   这个意思就是按PE的净雨和分段之后的5mm净雨，产流面积显然是不同的。所以FR值会有所变化。因为5mm对应的产流值不知道，所以没法直接用5mm算，除非从前面产流计算的时候算5mm就能停下来，可是前面的计算都是连续的过程，这样处理显然比较麻烦。
   所以才要寻找两个不同净雨之间的FR的对应关系。形式上看，和前面地下出流系数分段推求相似。也是做了线性化的处理，公式的意思是不产流面积的比例每小段的累乘与总的不产流面积比例一样。书上并没有给出很严谨的推论，只是直接给出了结论，所以这个地方**合理性存疑**！代码使用了这个公式，总有负值，需要**进一步调试**，如果确认是这有问题，需要说明这里计算不对！！
  
- PE<0的情况：
  书上没有给出这种情况的处理方法。PE<0时，流域没有产流面积，且前面产流计算的时段产流也是0，所以这里应该既是因为和前面的计算一致，也是本事计算原理就是在产流面积上定义的，所以应该是0，地下自由水蓄水水库应该是不泄流的。
