...
单位冲击响应就是**瞬时单位线**。单位矩形函数是时段净雨的形式了，单位净雨输入到系统的响应就是**时段单位线**。有了时段单位线，利用卷积的离散形式，和净雨过程做运算即可得到单元出口流量了，每个时段都这么算，最后将计算结果对应时段累加即可。公式如下所示：
...
这里涉及的线性代数可参考[视频1](https://www.bilibili.com/video/av6731067/?p=8)和[视频2](https://www.bilibili.com/video/av36568126/?p=16)。
...
由于单位线的定义是单位时段10mm净雨对应的出口断面流量，假设单位线时段变长了，按照单位线定义，是单位时段10mm净雨对应的流量，以前单位时段是$\triangle t_0$，现在是△t（△t>$\triangle t_0$），这时候如果还是直接使用单位线，意思就是认为△t内10mm净雨产生的流量，在每个△t上的大小还是原来单位线的大小，举例，比如原来1个小时10mm净雨产生的流量大小共三个时段，2-3-1，现在2个小时共10mm净雨产生的流量大小不变，即2个小时10mm净雨对应的流量是2（2h）-3（2h）-1（2h），注意和1h 5mm 2-3-1有区别，不过不管怎样，都显然是不合理的。总之，时段变化的话，原来时段的单位线肯定是不能用的。
...
对于瞬时单位线，由于实际中运算是离散的，所以一般都是先把瞬时单位线住哪换为S(t)曲线，然后用S曲线推求无因次的单位线，最后再转换为单位线。
