...
- [HydroDHM](https://github.com/OuyangWenyu/HydroDHM) - 可微分物理水文模型

## 示例结果

### SimpleLSTM 在 CAMELS-US 数据集上的表现

本示例展示了在完整 CAMELS-US 数据集上使用深度学习方法进行大规模径流预测的结果。该实验使用与 hydromodel 相同的数据基础设施（hydrodataset），展示了互补的建模方法。

**数据集配置：**
- **数据集**: CAMELS-US（美国本土 671 个流域）
- **训练期**: 1980-01-01 至 2004-12-31（25 年）
- **验证期**: 2005-01-01 至 2009-12-31（5 年）
- **测试期**: 2010-01-01 至 2014-12-31（5 年）
- **输入特征**: 共 30 个
  - 7 个时变气象变量（降水、潜在蒸散发、温度、太阳辐射等）
  - 23 个静态流域属性（地形、地质、植被、土壤特性）

**模型架构：**
- **模型类型**: SimpleLSTM（序列到序列，集成静态特征）
- **隐藏单元数**: 128 个 LSTM 单元
- **参数量**: 约 133K 可训练参数（模型大小：536 KB）
- **预测长度**: 365 天（1 年日尺度预测）

**硬件与运行时间：**
- **GPU**: NVIDIA 5070ti（16GB 显存）
- **框架**: PyTorch + CUDA 加速
- **训练时长**: 约 47 分钟（7 个 epoch，早停）
- **每个 epoch 平均时间**: 约 7.9 分钟
- **每个流域每个 epoch 时间**: 约 0.7 秒

**训练配置：**
- **优化器**: Adam（学习率：0.0005）
- **学习率调度器**: ExponentialLR（衰减因子：0.95）
- **损失函数**: RMSESum
- **批大小**: 256
- **早停**: 启用（耐心值：5 个 epoch）
...
**性能分布：**
- **优秀（NSE > 0.8）**: 169 个流域（25%）
- **良好（0.7 < NSE ≤ 0.8）**: 168 个流域（25%）
- **满意（0.5 < NSE ≤ 0.7）**: 168 个流域（25%）
- **不满意（NSE ≤ 0.5）**: 164 个流域（25%）

**关键发现：**
- **中位数 NSE 为 0.717**，表明模型在不同流域类型上表现强劲
- 成功处理异质性流域特征（从干旱到湿润气候，各种地形）
- 表现最佳的流域达到 **NSE > 0.85**，主要位于湿润的东北部地区
- 高效训练：671 个流域、25 年日尺度数据仅需 47 分钟

**来源**: 此示例来自 [HydroDHM](https://github.com/OuyangWenyu/HydroDHM) 项目。完整的实验细节、模型代码和复现说明，请参见 [LSTM CAMELS-US 示例文档](https://github.com/OuyangWenyu/HydroDHM/blob/main/docs/examples/lstm_camels_all_example.md)。
