...
def calibrate(args):
    ...
    data_dir = os.path.join('/home/ldaning/code/biye/hydro-model-xaj/hydromodel/example/model_run_wuxi7')
    kfold = [
    ...
    for fold in kfold:
        ...
        if (
            ...
            raise FileNotFoundError(
                "The data files are not found, please run datapreprocess4calibrate.py first."  #判断输入前的数据是否准备了
            )    
        ...
        if algo_info["name"] == "SCE_UA":        #方法一
            for i in range(len(data_info_train["basin"])):   #流域循环
                ...
                qsim = hydro_constant.convert_unit( #单位转换
                    qsim,
                    unit_now="mm/hour",
                    unit_final=hydro_constant.unit["streamflow"],
                    basin_area=basin_area,
                )
                qobs = hydro_constant.convert_unit(  #单位转换
                    data_test[365:, i : i + 1, -1:],
                    unit_now="mm/hour",
                    unit_final=hydro_constant.unit["streamflow"],
                    basin_area=basin_area,
                )
                ...
                show_test_result(
                    basin_id, test_date,qsim, qobs, save_dir=spotpy_db_dir,warmup_length=warmup,prcp=data_test[365:, i : i + 1, 0:1].flatten()
                )
