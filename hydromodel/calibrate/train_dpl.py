...
LastEditTime: 2023-06-04 21:16:47
...
from hydromodel.models.dpl_basic import SimpleLSTM
...
def get_dataloaders(cfg, chosen_basins, streamflow_ds, forcing_ds, attrs):
    ...
    sequence_length = cfg.model_params.model_param.seq_length
    ...
    tr_loader = DataLoader(
        ds_train, batch_size=batch_size, shuffle=True, collate_fn=collate_fn_seq_first
    )
    # Validation data
...
def train_model(config, basins, streamflow_ds, forcing_ds, attrs):
    # Initialize accelerator
    accelerator = Accelerator()
    # Sample hyper-parameters for learning rate, batch size, seed and a few other HPs
    lr = config.training_params.optim_params.lr
    ...
    input_size = len(OmegaConf.to_container(config.data_params.relevant_cols)) + len(
        OmegaConf.to_container(config.data_params.constant_cols)
    )
    # Number of LSTM cells in hidden layer
    hidden_size = config.model_params.model_param.hidden_size
    model = SimpleLSTM(
