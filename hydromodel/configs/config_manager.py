...
def load_hydro_settings() -> Dict[str, Any]:
    ...
    try:
        setting_path = os.path.join(os.path.expanduser("~"), "hydro_setting.yml")
...
def update_nested_dict(target_dict: Dict[str, Any], keys: List[str], value: Any) -> None:
    """
...
def get_default_data_path(data_source_type: str, hydro_settings: Dict[str, Any]) -> str:
    """
...
class ConfigManager:
        ...
        model_defaults = {
            ...
            "categorized_unit_hydrograph": {
                "uh_categories": {
                    ...
                    "medium": [0.02, 0.08, 0.15, 0.20, 0.22, 0.18, 0.10, 0.06,
                              0.04, 0.03, 0.02, 0.01, 0.01, 0.005, 0.003, 0.002],
                    "large": [0.01, 0.03, 0.08, 0.12, 0.16, 0.18, 0.15, 0.12,
                             0.08, 0.06, 0.05, 0.04, 0.03, 0.02, 0.015, 0.01,
                             0.008, 0.006, 0.004, 0.003, 0.002, 0.001, 0.001, 0.001]
                },
                "thresholds": {
                    "small_medium": 10.0,
                    "medium_large": 25.0
            ...
            "gr4j": {
                "X1": 350.0, "X2": 0.0, "X3": 90.0, "X4": 1.2
            },
            "gr6j": {
                "X1": 350.0, "X2": 0.0, "X3": 90.0, "X4": 1.2, "X5": 0.5, "X6": 3.0
            },
        }
    ...
    @staticmethod
    def update_config_from_args(base_config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
        """
        ...
        if hasattr(args, 'data_source_path') and args.data_source_path is not None:
            ...
            config["data_cfgs"]["data_source_path"] = get_default_data_path(data_type, hydro_settings)
        ...
        if "simulation_cfgs" in config:
            ...
            if hasattr(args, 'experiment_name') and args.experiment_name is not None:
                config["simulation_cfgs"]["experiment_name"] = args.experiment_name
        ...
        if "training_cfgs" in config and config["training_cfgs"]["experiment_name"] is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            model_name = config["model_cfgs"]["model_name"]
            algorithm = config["training_cfgs"]["algorithm_name"]
            config["training_cfgs"]["experiment_name"] = f"{model_name}_{algorithm}_{timestamp}"
        
        if "simulation_cfgs" in config and config["simulation_cfgs"]["experiment_name"] is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            model_name = config["model_cfgs"]["model_name"]
            config["simulation_cfgs"]["experiment_name"] = f"{model_name}_simulation_{timestamp}"
    ...
    @staticmethod
    def create_calibration_config(config_file: Optional[str] = None, 
    ...
    @staticmethod
    def create_simulation_config(config_file: Optional[str] = None,
                               args: Optional[argparse.Namespace] = None,
                               updates: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
