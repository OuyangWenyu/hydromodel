...
class UnifiedConfig:
    ...
    def _set_data_defaults(self):
        ...
        data_defaults = {
            "data_type": "selfmadehydrodataset",
            "data_dir": None,
            "basin_ids": [],
            "warmup_length": 365,
            "time_periods": {
    ...
    def get_algorithm_config(self) -> Dict:
        """Get algorithm configuration in format expected by unified calibrate function."""
        training_cfg = self.training_cfgs
        
        config = {
            "name": training_cfg["algorithm_name"]
        }
        
        # Add algorithm-specific parameters based on algorithm type
        if "algorithm_params" in training_cfg:
            params = training_cfg["algorithm_params"]
            algorithm_name = training_cfg["algorithm_name"]
            
            if algorithm_name == "SCE_UA":
                config.update({
                    "random_seed": params.get("random_seed", 1234),
                    "rep": params.get("rep", 1000),
                    "ngs": params.get("ngs", 1000),
                    "kstop": params.get("kstop", 50),
                    "peps": params.get("peps", 0.1),
                    "pcento": params.get("pcento", 0.1)
                })
            elif algorithm_name == "genetic_algorithm":
                config.update({
                    "random_seed": params.get("random_seed", 1234),
                    "pop_size": params.get("pop_size", 80),
                    "n_generations": params.get("n_generations", 50),
                    "cx_prob": params.get("cx_prob", 0.7),
                    "mut_prob": params.get("mut_prob", 0.2),
                    "save_freq": params.get("save_freq", 5)
                })
    ...
    def get_loss_config(self) -> Dict:
        """Get loss configuration in format expected by unified calibrate function."""
        eval_cfg = self.evaluation_cfgs
        
        return {
            "type": eval_cfg.get("loss_type", "time_series"),
            "obj_func": eval_cfg.get("objective_function", "RMSE"),
            "events": eval_cfg.get("events_config")
        }
