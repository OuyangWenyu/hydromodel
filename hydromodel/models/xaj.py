...
def calculate_w_storage(um, lm, dm,
                        ...
                        pe, r) -> tuple[np.array, np.array, np.array]:
    """
...
def generation(p_and_e,
               b, im, um, lm, dm, c,
               wu0=None, wl0=None, wd0=None) -> tuple:
    """
...
def sources(pe, r,
    ...
    fr = np.where(pe > 0.0, r / pe, np.full(r.shape, 0.0))
    if np.isnan(fr).any():
        raise ArithmeticError("Please check pe's data! there may be 0.0")
    # For free water storage, both （流域水文模拟）WHS's sample code and HF use fr0 * s0 / fr.
    # The reason they use the formula may be that they treat "s" as an instantaneous state value.
    # I don't know why. Here I think it's simpler and clearer to treat "s" as a mean value.
    # Hence s0 is just the free water storage in this period.
    ss = np.minimum(s0, sm - precision)
    ...
    s = np.where(fr > 0.0, ss + (r - rs) / fr, ss)
    if np.isnan(s).any():
    ...
    s1 = np.where(fr > 0.0, s * (1 - ki - kg), s)
    s1 = np.clip(s1, a_min=np.full(s.shape, 0.0), a_max=sm)
    return (rs, ri, rg), s1
...
def uh_gamma(a, theta, len_uh=15):
    ...
    # len_f, batch, feature
    t = np.expand_dims(np.swapaxes(np.tile(np.arange(0.5, len_uh * 1.0), (m[1], 1)), 0, 1), axis=-1)
...
def xaj(p_and_e,
        ...
        warmup_length=30,
        source_type="sources",
        source_book="ShuiWenYuBao") -> Union[tuple, np.array]:
    """
    run XAJ model
    ...
    if warmup_length > 0:
        ...
        _, *w0, s0, qi0, qg0 = xaj(p_and_e_warmup, params, return_state=True, kernel_size=kernel_size, warmup_length=0)
    else:
        w0 = (0.5 * um, 0.5 * lm, 0.5 * dm)
        s0 = 0.5 * sm
        qi0 = np.full(ci.shape, 0.001)
        qg0 = np.full(cg.shape, 0.001)
        if source_type == "sources5mm":
            # need fr0
            s0 = (s0, np.full(ex.shape, 0.001))
    ...
    for i in range(inputs.shape[0]):
        if i == 0:
            (r, rim, e, pe), w = generation(inputs[i, :, :], b, im, um, lm, dm, c, *w0)
            if source_type == "sources":
                (rs, ri, rg), s = sources(pe, r, sm, ex, ki, kg, s0)
            elif source_type == "sources5mm":
                (rs, ri, rg), s = sources5mm(pe, r, sm, ex, ki, kg, *s0, book=source_book)
            else:
                raise NotImplementedError("No such divide-sources method")
        else:
            (r, rim, e, pe), w = generation(inputs[i, :, :], b, im, um, lm, dm, c, *w)
            if source_type == "sources":
                (rs, ri, rg), s = sources(pe, r, sm, ex, ki, kg, s)
            elif source_type == "sources5mm":
                (rs, ri, rg), s = sources5mm(pe, r, sm, ex, ki, kg, *s, book=source_book)
            else:
                raise NotImplementedError("No such divide-sources method")
        runoff_ims_[i, :] = rim
        rss_[i, :] = rs
