...
LastEditTime: 2025-02-26 16:19:53
LastEditors: zhuanglaihong
Description: 
FilePath: /zlh/hydromodel/scripts/calibrate_gr5j.py
...
def calibrate(args):
    ...
    if cv_fold <= 1:
        ...
        calibrate_by_sceua(
            ...
            p_and_e,
            qobs,
            os.path.join(where_save, "sceua_gr5j"),
            warmup,
            model=model_info,
            algorithm=algo_info,
            loss=loss_info,
        ...
        for i in range(cv_fold):
            ...
            calibrate_by_sceua(
                ...
                p_and_e_cv,
                qobs_cv,
                os.path.join(where_save, f"sceua_gr5j_cv{i+1}"),
                warmup,
                model=model_info,
                algorithm=algo_info,
                loss=loss_info,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run hydro-model-gr5j models with the CAMELS dataset"
    )
    parser.add_argument(
        "--data_type",
        dest="data_type",
