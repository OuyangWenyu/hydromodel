...
def calculate_evap(lm, c,
    ...
    out : float
        eu,el,ed:流域时段三层蒸散发
    """
    eu = jnp.where(wu0 + prcp >= pet, pet, wu0 + prcp)
    ed = jnp.where((wl0 < c * lm) & (wl0 < c * (pet - eu)), c * (pet - eu) - wl0, 0)
    el = jnp.where(wu0 + prcp >= pet,
                   0,
                   jnp.where(wl0 >= c * lm, (pet - eu) * wl0 / lm,
                             jnp.where(wl0 >= c * (pet - eu), c * (pet - eu), wl0)))
    return eu, el, ed
...
def calculate_prcp_runoff(b, im, wm,
    ...
    r_cal = jnp.where(pe + a < wmm,
                      pe - (wm - w0) + wm * (1 - (a + pe) / wmm) ** (1 + b),
                      pe - (wm - w0))
    r = jnp.maximum(r_cal, 0)
    r_im_cal = pe * im
    r_im = jnp.maximum(r_im_cal, 0)
    return r, r_im
...
def streamflow_step(params, state_variables, forcings):
    ...
    prcp = jnp.maximum(prcp, 0)
    pet = jnp.maximum(pet, 0)
    # wm
    wm = um + lm + dm

    w0_ = wu0 + wl0 + wd0
    # 注意计算a时，开方运算，偶数方次时，根号下不能为负数，所以需要限制w0的取值，这也是物理意义上的要求
    w0 = jnp.where(w0_ > wm, wm, w0_)
    ...
    pe = jnp.maximum(prcp_difference, 0)
    ...
    qt_history = jnp.roll(qt_history, 1)
    qt_history = index_update(qt_history, index[0], qt)
    # not sure if this could be used with "grad" for l.
    q = cs * q0 + (1 - cs) * qt_history[l]
    # q = cs * q0 + (1 - cs) * qt # just for test
    # if cannot, use unit hydrograph
    # q = jnp.convolve(qt_history, UH)
...
def simulate_streamflow(prcp, pet, area,
    ...
    """
    area = jnp.full(len(prcp), area)
    # sequence-first variables are needed for loop calculation
    forcings = jnp.moveaxis(jnp.array([prcp, pet, area]), 1, 0)
