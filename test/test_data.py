...
@pytest.fixture()
def txt_file():
    return os.path.join(definitions.ROOT_DIR, "hydromodel", "example", '01013500_lump_p_pe_q.txt')
...
@pytest.fixture()
def npy_file():
    return os.path.join(definitions.ROOT_DIR, "hydromodel", "example", 'basins_lump_p_pe_q.npy')
...
def test_save_data(txt_file, json_file, npy_file):
    ...
    data_info = OrderedDict({"time": data['date'].values.tolist(), "basin": ["01013500"], "variable": variables})
