...
def test_gr4j_model():
    ...
    time_steps = 10
    n_basins = 2
    p_and_e = np.random.rand(
        time_steps, n_basins, 2
    )  # [time, basin, variable]
    ...
    # 测试返回状态值的情况
    streamflow, ets, s, r = gr4j(
        p_and_e, parameters, warmup_length=0, return_state=True
    )
...
def test_gr4j_continuity():
    ...
    # 先进行预热
    warmup_length = 20
    qsim_full, _, s_full, r_full = gr4j(
        p_and_e, params, warmup_length=warmup_length, return_state=True
    )
