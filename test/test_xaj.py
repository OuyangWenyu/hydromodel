...
from src.calibrate.calibrate_xaj_sceua import calibrate_xaj_sceua, SpotSetup
...
class XajTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        qobs = np.expand_dims(test_data[['Discharge[ls-1]']].values, axis=0)
        basin_area = 1.783
        # trans l/s to mm/day
        self.qobs = qobs * 1e-3 / (basin_area * km2tom2) * mtomm * daytos
        # three dims: batch (basin), sequence (time), feature (variable)
        self.p_and_e = np.expand_dims(p_and_e_df.values, axis=0)
    ...
    def test_show_calibrate_sceua_result(self):
        spot_setup = SpotSetup(self.p_and_e, self.qobs, init_states=self.states,
                               obj_func=spotpy.objectivefunctions.rmse)
        show_calibrate_result(spot_setup, "SCEUA_xaj_hymoddata")
        plt.show()
