...
from hydromodel.models.xaj_songliao import (
    xaj_songliao,
    load_xaj_data_from_json,
)
...
def test_xaj_with_real_data():
    ...
    try:
        ...
        json_file = os.path.join(
            os.path.dirname(__file__), "..", "data", "xaj_data.json"
        )
        ...
        print("\n三水源分量分析:")
        print(
            f"地表径流: {total_rs:.2f} mm ({total_rs/total_components*100:.1f}%)"
        )
        print(
            f"壤中流: {total_ri:.2f} mm ({total_ri/total_components*100:.1f}%)"
        )
        print(
            f"地下径流: {total_rg:.2f} mm ({total_rg/total_components*100:.1f}%)"
        )
