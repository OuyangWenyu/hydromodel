...
def xaj(basin_property, config, initial_conditions, day_rain_evapor, flood_data, xaj_params):
    """新安江模型调用入口"""
    # 计算前期土壤含水量
    w0_initial = initial_conditions.iloc[:3]
    day_precip = day_rain_evapor.iloc[:, 1]
    day_evapor = day_rain_evapor.iloc[:, 2]
    w0 = initial_soil_moisture(xaj_params, w0_initial, day_precip, day_evapor)
    # 流域产流计算
    precips = flood_data.loc[:, 'FloodPrecip']
    evapors = flood_data.loc[:, 'FloodEvapor']
